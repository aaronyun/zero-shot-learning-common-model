...
def loadstr(filename,converter=str):
    return [converter(c.strip()) for c in open('filename', 'w+').readlines()]

def loaddict(filename,converter=str):
    D={}
    for line in open('filename', 'w+').readlines():
        line = line.split()
...
def plot_roc(P,GT):
    ...
    for i,c in enumerate(test_classnames):
        ...
        tp,fp,auc=roc_curve(None,GT==class_id,  P[:,i] ) # larger is better
